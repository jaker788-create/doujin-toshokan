...
// sr builds a search result from an int id for test ergonomics; the neutral model
// stores ids as strings (nhentai's are numeric), so it formats on the way in.
func sr(id int64, en, jp string, pages, favs int) source.SearchResult {
	return source.SearchResult{ID: strconv.FormatInt(id, 10), EnglishTitle: en, JapaneseTitle: jp, NumPages: pages, NumFavorites: favs}
}

// gid parses a candidate's string gallery id back to the int the tests assert on.
func gid(c Candidate) int64 { id, _ := strconv.ParseInt(c.Gallery.ID, 10, 64); return id }

// noLang is a candLang resolver reporting no language for any candidate.
func noLang(source.SearchResult) string { return "" }

// langByID builds a candLang resolver from a gallery-id -> language map.
func langByID(m map[int64]string) func(source.SearchResult) string {
	return func(r source.SearchResult) string { id, _ := strconv.ParseInt(r.ID, 10, 64); return m[id] }
}

// noArtist / artistAll are artistMatch resolvers reporting the obvious for any candidate.
func noArtist(source.SearchResult) bool  { return false }
func artistAll(source.SearchResult) bool { return true }

func applyIDs(cs []Candidate) []int64 {
	ids := make([]int64, len(cs))
	for i, c := range cs {
		ids[i], _ = strconv.ParseInt(c.Gallery.ID, 10, 64)
	}
	return ids
}
...
func TestDecideAutoRomajiLocalEnglishOnline(t *testing.T) {
	cands := ScoreAll([]string{"Karakishi Youhei-dan Compilation"}, 50, "", []source.SearchResult{
	...
	if gid(d.Ranked[0]) != 10 {
		t.Errorf("top = %s, want 10", d.Ranked[0].Gallery.ID)
	}
...
func TestDecideAutoJapaneseLocalJapaneseOnline(t *testing.T) {
	cands := ScoreAll([]string{"からきし傭兵団"}, 20, "", []source.SearchResult{
...
func TestDecideMergesDuplicateExactPageVariants(t *testing.T) {
	...
	cands := ScoreAll([]string{"alpha beta gamma"}, 10, "", []source.SearchResult{
...
func TestDecideMergeWindowExcludesWeakerTitle(t *testing.T) {
	...
	cands := ScoreAll([]string{"one two three four five six"}, 10, "", []source.SearchResult{
...
func TestDecideSameLanguageBecomesPrimary(t *testing.T) {
	...
	if gid(d.Apply[0]) != 10 {
		t.Errorf("primary = %s, want 10 (same language wins over favorites)", d.Apply[0].Gallery.ID)
	}
	if len(d.Apply) != 2 {
...
func TestDecideLanguageMismatchStillAutoApplies(t *testing.T) {
	...
	cands := ScoreAll([]string{"lonely title"}, 30, "english", []source.SearchResult{
...
func TestDecideReviewWhenTitleTooWeak(t *testing.T) {
	...
	cands := ScoreAll([]string{"alpha beta"}, 10, "", []source.SearchResult{
...
func TestDecideStrongTitleAutoDespiteFarPages(t *testing.T) {
	...
	cands := ScoreAll([]string{"exact title match"}, 10, "", []source.SearchResult{
	...
	weak := ScoreAll([]string{"alpha beta"}, 10, "", []source.SearchResult{
...
func TestDecideArtistMatchAutoDespitePageDelta(t *testing.T) {
	...
	cands := ScoreAll([]string{"Diploma Mill"}, 24, "", []source.SearchResult{
...
func TestDecideArtistExactPageLowTitle(t *testing.T) {
	...
	if d := Decide(ScoreAll([]string{local}, 20, "", []source.SearchResult{cand}, noLang, artistAll)); d.Action != ActionAuto {
		t.Errorf("action = %q, want auto (artist + exact pages)", d.Action)
	}
	if d := Decide(ScoreAll([]string{local}, 20, "", []source.SearchResult{cand}, noLang, noArtist)); d.Action != ActionReview {
...
func TestDecideStrongTitleBarBoundary(t *testing.T) {
	...
	cands := ScoreAll([]string{"Gift"}, 10, "", []source.SearchResult{
		sr(10, "Gift", "", 40, 5),
	}, noLang, noArtist)
	if d := Decide(cands); d.Action != ActionReview {
...
func TestScoreCarriesArtistMatchAndTokens(t *testing.T) {
	...
	cs := ScoreAll([]string{"x"}, 10, "", []source.SearchResult{sr(1, "x", "", 10, 0)}, noLang, nil)
	if cs[0].ArtistMatch {
...
func TestDecideReviewOnEmptyAndGarbage(t *testing.T) {
	...
	cands := ScoreAll([]string{}, 0, "", []source.SearchResult{sr(10, "anything", "", 5, 1)}, noLang, noArtist)
	if d := Decide(cands); d.Action != ActionReview {
